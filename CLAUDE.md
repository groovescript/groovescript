...
- Per-bar subdivision is inferred from bar content, not declared. The slot grid is the LCM of every label's needs and every tuplet group's `actual` ratio (so a bar mixing 16ths with sextuplets uses 12 slots per beat, a sextuplet bar uses 6, etc.). Mixing two different tuplet kinds on the same beat across instruments is rejected
- Tuplet groups: `<beat>{<kind> <slots>}` (e.g. `2{sextuplet 1, 4 accent}`) anchor a tuplet at a beat. Kinds are `triplet`/`quintuplet`/`sextuplet`/`septuplet`/`nonuplet`. `/8` qualifier (e.g. `2&{triplet/8 1, 3}`) makes the span half a beat. Per-beat tuplet annotations live on `IRBar.beat_tuplets` (`("full", actual, normal)` or `("halves", left, right)`) and drive LilyPond / MusicXML emission
- Star shorthand `*<kind>[/N]` (e.g. `*sextuplet`, `*triplet/8`) fills the bar with one named tuplet per beat (or two per beat for `/8`). Lives on `StarSpec.tuplet_kind` / `tuplet_span`
- Count strings accept inline tuplet groups: `count: "1 2{sextuplet 1, 2, 3, 4, 5, 6} 3 4"`. Whole-beat anchors only at the count layer; sub-beat tuplets need the pattern-line form. The count parser emits synthetic `~T<anchor>_<slot>_<actual>_<normal>` labels that `_beat_label_to_fraction` decodes
- Fill bars have independent subdivision from surrounding groove bars; they also carry their own `IRFillBar.beat_tuplets`, merged into the bar via `_merge_fill_beat_tuplets`
...
- A section-level `variation at bar N:` block may carry `time_signature: N/M` to override that bar's meter within an otherwise-uniform section. Because the beat grid no longer matches the surrounding groove, the bar starts from an empty canvas rather than being diffed against it: `remove`/`replace`/`modify` are rejected (nothing pre-existing to target), so only `add`/`substitute` are allowed. Zero actions renders as a plain rest in the new meter (`Variation.time_signature`, `_compile_meter_variation_bar` in `compiler.py`). Cannot share a bar with a fill or a `break on bar` (`_validate_meter_variation_conflicts`); not supported inside groove `extend:` bodies or top-level `variation "name":` defs. `Variation.is_reference` (not "empty actions") is what distinguishes a bodyless `variation "name" at bar N` library/top-level reference from a named inline block that merely has no regular actions (e.g. a `time_signature`-only block)
...
- A section is either classic (`bars:` + `groove:`) or play-list (`play:`) -- `groove:`/`repeat:` are mutually exclusive with `play:`. A `bars:` line may accompany `play:` as a redundant length assertion; the compiler errors if it doesn't equal the play block's expanded bar count
...
- Placeholder grooves are TBD slots that render as empty bars with a boxed rehearsal label and emit silence in MIDI/MusicXML. Three forms: top-level `groove placeholder "name"` (no body, no colon), section sole `groove: placeholder ["label"]`, and `play:` items `groove placeholder ["label"] xN`. Inside `play:` only, an undefined groove name auto-promotes to a named placeholder using the reference name as label; outside `play:`, an undefined name remains an error. Nameless placeholder spans within one section get numeric suffixes (`"<Section> groove 1"`, `"<Section> groove 2"`, …) only when there's more than one. Variations targeting a placeholder bar are an error; `extend:` cannot reference a placeholder. Fills and cues overlay placeholder bars normally
...
uv run groovescript compile input.gs -o output.ly --no-compact  # Disable default collapsing of identical bars across implicit phrase boundaries
